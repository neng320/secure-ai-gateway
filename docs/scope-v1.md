...
| SEC-003 | 🟠 **VERIFYING — P1-04.2.1 gate delivery correction**（2026-08-29 复验发现 P1-04.1 的静态 runtime-log tripwire 因错误的 package-relative 路径实际扫描了 0 个生产文件——测试覆盖 false-green，非新运行时泄漏；runtime canary 与全部运行时修正本身 PASS）。Request logging defaults to metadata-only. Prompt/request/response bodies and raw upstream errors are not persisted. Optional diagnostic request-body capture is explicit, bounded, memory-only and hard-expiring. Legacy persisted request content is rejected at startup and has an explicit offline scrub path. **证据链**：P1-04A 特征化（6 KNOWN-VULN 红转绿）→ P1-04B metadata-only（RequestRecord 恒空强制/RequestID 128-bit 全链一致/bounded ErrorCode/HTML+WS 正文移除）→ P1-04C MEMORY-ONLY 诊断捕获（≤24h/64KiB/1000 硬上限/真实 timer）→ P1-04D legacy fail-closed + 双确认不可逆 scrub（WAL→DELETE 拍平+VACUUM 重写）→ **P1-04.1**：provider DEBUG 正文 sink 根除（6 处）+ scrub exclusive/busy fail-closed + capture 真实内存上界 + 过期配置 fail-safe → **P1-04.2**：FetchModels URL/headers（含 Authorization Bearer Provider Key）runtime log sink 根除、vLLM 完整 URL/proxy 凭据值/url.Error 根除（仅 proxy_ignored 布尔与 bounded code）、client.go 网关 Key 前缀根除、全 provider/service/handler 静态 tripwire（body/header/APIKey/proxy 模式）+ DEBUG=1 与 proxy 凭据 canary runtime Gate、scrub 升级为**持有式独占**（单一专用连接 locking_mode=EXCLUSIVE，锁保持至 VACUUM/verify 完成后释放；所有权保留属性测试：持有期并发者 BUSY/释放后恢复）。恢复点 tag `secure-gateway-p1-request-log-privacy.2`（`.0`/`.1` 保留为历史 checkpoint）。 | 完整 Prompt 默认持久化到 `request_logs.request_body` | baseline-audit §1.9 + docs/p1-04-request-log-characterization.md | 已满足 | P1-04A~D + P1-04.1/04.2 ✅ |
