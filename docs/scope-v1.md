...
| SEC-002 | 🟠 **MIGRATION IMPLEMENTED / REAL EXECUTION PENDING**（2026-08-29；P1-03A Characterization ✅ → P1-03B/B.1 Crypto Foundation ✅ → **P1-03C0~C3 迁移实现与 Runtime/UI 切换 ✅（含 P1-03C2.1 迁移前置修正）**：Domain Secret Manager（AAD 领域隔离）、additive 加密存储+启动 preflight fail-closed、离线迁移引擎（BACKUP→[SCHEMA-ADD]→PREPARE→VERIFY→FINALIZE，纯读 loader/DB fail-closed/备份严格先于 mutation/仅 ADD COLUMN）、Gemini sink header 化、runtime/persistent 双视图（持久化恒 envelope-only）、Admin Create/Update 加密语义（blank=保留/新 key=加密替换/显式清除）、编辑表单不再回显材料。证据：Migration Simulation Security Gate（7 全栈用例+fixture A-F 9 用例+C2.1 前置 6 用例+P1-03C3.1 Final Staging 9 用例）@ develop `e34f614`，恢复点 tag `secure-gateway-p1-secret-migration-staging.1`（进入 P1-03D 的正式基线；`.0` staging tag 保留为修正前 recovery point）。**尚未对任何真实 config.yaml/gateway.db 执行迁移——SEC-002 不得 CLOSED**） | Provider Key 明文存储（SQLite `backend_api_key` + config.yaml） | baseline-audit §1.2 + docs/p1-03-key-flow-characterization.md | AEAD（AES-256-GCM）加密落库，主密钥来自环境变量/secret 文件；库内仅 ciphertext+nonce+key_id。基础设施+迁移+切换均已就绪并经模拟验证 | P1-03D（真实迁移执行 + Plaintext Eradication Gate，待人工 Review 后） |
