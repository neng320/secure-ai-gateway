...
| SEC-003 | ✅ **CLOSED**（2026-08-29，P1-04 + P1-04.1 + P1-04.2 + **P1-04.2.1 gate delivery correction**）。Request logging defaults to metadata-only. Prompt/request/response bodies and raw upstream errors are not persisted. Optional diagnostic request-body capture is explicit, bounded, memory-only and hard-expiring. Legacy persisted request content is rejected at startup and has an explicit offline scrub path. **证据链**：P1-04A 特征化（6 KNOWN-VULN 红转绿）→ P1-04B metadata-only（RequestRecord 恒空强制/RequestID 128-bit 全链一致/bounded ErrorCode/HTML+WS 正文移除）→ P1-04C MEMORY-ONLY 诊断捕获（≤24h/64KiB/1000 硬上限/真实 timer）→ P1-04D legacy fail-closed + 双确认不可逆 scrub → **P1-04.1**（provider DEBUG 正文 sink 6 处根除/scrub exclusive+busy fail-closed/capture 真实内存上界/过期配置 fail-safe）→ **P1-04.2**（FetchModels URL+headers 含 Authorization Bearer Provider Key 根除、vLLM URL/proxy 凭据值/url.Error 根除、client.go key 前缀根除、持有式独占重写+所有权保留属性测试）→ **P1-04.2.1**：repaired a zero-file static tripwire false-green; the gate now resolves the repository root explicitly, asserts its coverage set (PRODUCTION_FILES_SCANNED=18, required 7/7), and uses AST-based sink inspection（多行 CallExpr 覆盖+len() 放行+URL.Path 放行+合成 fixture 5 抓 4 放）+ WAL ownership 实际分支测试（WAL→独占→DELETE 拍平→VACUUM→双清零）。恢复点 tag `secure-gateway-p1-request-log-privacy.3`（.0/.1/.2 保留为历史 checkpoint）。 | 完整 Prompt 默认持久化到 `request_logs.request_body` | baseline-audit §1.9 + docs/p1-04-request-log-characterization.md | 已满足 | P1-04A~D + P1-04.1/04.2/04.2.1 ✅ |
