...
| SEC-003 | ✅ **CLOSED**（2026-08-29，P1-04 + **P1-04.1 final corrections**）。Request logging defaults to metadata-only. Prompt/request/response bodies and raw upstream errors are not persisted. Optional diagnostic request-body capture is explicit, bounded, memory-only and hard-expiring. Legacy persisted request content is rejected at startup and has an explicit offline scrub path. **P1-04.1 关闭证据**：① provider DEBUG request/response body runtime log 通道根除（openai_compat/ollama/vllm 共 6 处 sink，DEBUG 仅剩 provider/model/status/bytes metadata）+ DEBUG=1 canary 双端 Gate + 静态 tripwire；② scrub exclusive/busy fail-closed（wal_checkpoint TRUNCATE 遵守 SQLite Busy 契约 + BEGIN EXCLUSIVE 独占探测，均在 mutation 之前；WAL/journal 双模式并发 Gate：OFFLINE_REQUIRED + 原值不变 → 释放后逻辑+raw bytes 清零）；③ capture 真实内存上界（min(len,maxBytes) 分配 + 同 ID 覆盖置零）；④ 过期 capture 配置 fail-safe disable（非 fatal）。此前 P1-04 证据链与 6 个 KNOWN-VULN 红转绿见 git 历史与 docs/p1-04-request-log-characterization.md。恢复点 tag `secure-gateway-p1-request-log-privacy.1`（旧 tag 保留为 pre-P1-04.1 checkpoint，其 tree 完整但指向 PR head）。 | 完整 Prompt 默认持久化到 `request_logs.request_body` | baseline-audit §1.9 + docs/p1-04-request-log-characterization.md | 已满足 | P1-04A~D + P1-04.1 ✅ |Request logging defaults to metadata-only. Prompt/request/response bodies and raw upstream errors are not persisted. Optional diagnostic request-body capture is explicit, bounded, memory-only and hard-expiring. Legacy persisted request content is rejected at startup and has an explicit offline scrub path. 关闭链：P1-04A 特征化（6 KNOWN-VULN 红转绿）→ P1-04B metadata-only（RequestRecord 结构化 API 恒空强制/RequestID crypto-rand 128-bit 全链一致/bounded ErrorCode/Dashboard+Client Detail+WS 全部移除正文与 raw error/fallback runtime log 仅 bounded code）→ P1-04C MEMORY-ONLY 诊断捕获（显式 opt-in/≤24h 硬过期/64KiB·1000 硬上限/真实 timer 清空/Admin on-demand no-store）→ P1-04D legacy 残留启动 fail-closed + 双确认不可逆离线 scrub（VACUUM 重写+sidecar 检查）。证据：Prompt Privacy Security Gate（全栈 5 用例：DEFAULT MODE 三类请求磁盘/日志 0 canary/DIAGNOSTIC/BOUNDS/RUNTIME ERROR/静态 tripwire）@ develop，恢复点 tag `secure-gateway-p1-request-log-privacy` | 完整 Prompt 默认持久化到 `request_logs.request_body` | baseline-audit §1.9 + docs/p1-04-request-log-characterization.md | 已满足 | P1-04A~D ✅ |
